...
@app.get("/vectorize-images")
async def vectorize_images():
    documents = collection.find()
    results = []
    for doc in documents:
        image_url = doc.get('image')  # Updated to use the 'image' field
        if image_url:
            try:
                ...
                collection.update_one({'_id': doc['_id']}, {'$set': {'image_embedding3': image_embedding}})
